...
class IQADataset_jp2k(Dataset):
    def __init__(self, conf, exp_id=0, status='train', loader=default_loader):
        ...
        testindex = index[int((1-test_ratio) * len(index)):]
        train_index, val_index, test_index = [],[],[]
        for i in range(len(ref_ids_jp2k)):
            train_index.append(i) if (ref_ids_jp2k[i] in trainindex) else \
                test_index.append(i) if (ref_ids_jp2k[i] in testindex) else \
                    val_index.append(i)
        if status == 'train':
            self.index = train_index
            print("# Train Images: {}".format(len(self.index)))
    ...
    def __getitem__(self, idx):
        return ( (self.patches_dis[idx],self.patches_gradient[idx],self.distance[idx],self.entropy[idx]), (torch.Tensor([self.label[idx],]),
                torch.Tensor([self.label_std[idx],])))


#jpeg
class IQADataset_jpeg(Dataset):
    def __init__(self, conf, exp_id=0, status='train', loader=default_loader):
        self.loader = loader
        im_dir = conf['LIVEjpeg']['im_dir']
        self.patch_size = conf['patch_size']
        self.stride = conf['stride']
        datainfo = conf['LIVEjpeg']['datainfo']
        Info = h5py.File(datainfo)
        index = Info['index'][:, int(exp_id) % 1000]
        ref_ids = Info['ref_ids'][0, :]
        test_ratio = conf['test_ratio']
...
class IQADataset_wn(Dataset):
    def __init__(self, conf, exp_id=0, status='train', loader=default_loader):
        self.loader = loader
        im_dir = conf['LIVEwn']['im_dir']
        self.patch_size = conf['patch_size']
        self.stride = conf['stride']
        datainfo = conf['LIVEwn']['datainfo']
        Info = h5py.File(datainfo)
        index = Info['index'][:, int(exp_id) % 1000]
        ref_ids = Info['ref_ids'][0, :]
        test_ratio = conf['test_ratio']
    ...
    def __getitem__(self, idx):
        return ((self.patches_dis[idx], self.patches_dis[idx], self.distance[idx], self.entropy[idx]),
                (torch.Tensor([self.label[idx], ]),
                 torch.Tensor([self.label_std[idx], ])))


#gblur
class IQADataset_blur(Dataset):
    def __init__(self, conf, exp_id=0, status='train', loader=default_loader):
        self.loader = loader
        im_dir = conf['LIVEblur']['im_dir']
        self.patch_size = conf['patch_size']
        self.stride = conf['stride']
        datainfo = conf['LIVEblur']['datainfo']
        Info = h5py.File(datainfo)
        index = Info['index'][:, int(exp_id) % 1000]
        ref_ids = Info['ref_ids'][0, :]
        test_ratio = conf['test_ratio']
...
class IQADataset_ff(Dataset):
    def __init__(self, conf, exp_id=0, status='train', loader=default_loader):
        self.loader = loader
        im_dir = conf['LIVEff']['im_dir']
        self.patch_size = conf['patch_size']
        self.stride = conf['stride']
        datainfo = conf['LIVEff']['datainfo']
        Info = h5py.File(datainfo)
        index = Info['index'][:, int(exp_id) % 1000]
        ref_ids = Info['ref_ids'][0, :]
        test_ratio = conf['test_ratio']
